...
| `tasks`         | all      | Task list with role-specific tabs (employee: assigned/created, admin: assigned/all), client-side search/filter/sort, live countdown chip on list/details via adaptive screen-level ticker, details, create/edit/delete by creator or admin, status updates (assignee) |
