...
## 2026-06-29 — Second audit fixes for v2.0.0 Collections (post-c443be8)

- **Decision**: Applied 4 additional fixes identified in a second pre-merge audit after commit `c443be8`. (1) `handover_verified`/`handover_discrepancy` in-app body now uses `adminName · settledAmount` — the H2 fix added `collectorName` to the payload but that field is the recipient's own name, which is useless; `adminName` and `settledAmount` were already in the payload and are the meaningful content. (2) Added `visits: [promiseToPay.status ASC, promiseToPay.promisedDate ASC]` composite index — the `checkBrokenPtps` compound equality+range query had been throwing `FAILED_PRECONDITION` every day since PR 5; no broken PTPs were ever marked. (3) Added `installments: [status ASC, dueDate ASC]` collection-group index — `sendInstallmentDueReminders` had been silently failing since PR 4 for the same reason. (4) `HandoverInitScreen` now filters `payState.payments` to exclude items where `handoverId != null`, preventing a collector from selecting an already-in-handover payment for a second handover. Also reconciled 6 indexes that existed in remote Firestore but were absent from `firestore.indexes.json` (`notifications: [userId, createdAt]`; `task_logs: [taskId, performedAt]`; `tasks: [assignedTo, createdAt]`; `tasks: [assignedBy, createdAt]`; `tasks: [assignedTo, dueDate]`; `tasks: [dueDate, status]`) — these were preserved by the deploy.
- **Reason**: Owner requested a second self-review after the first audit was applied. The two missing indexes were the most critical finding: both schedulers had been silently no-ops since they were first deployed. The in-app notification body correction and double-handover guard are data-integrity improvements before the financially sensitive feature reaches production.
- **Impact**: `firebase deploy --only firestore:indexes` was run and succeeded. `checkBrokenPtps` and `sendInstallmentDueReminders` will now function correctly on their next scheduled runs.
- **Owner**: Mohamed Odeh.
- **Related**: `firestore.indexes.json`, `notifications_screen.dart`, `handover_init_screen.dart`.

## 2026-06-29 — Pre-merge audit fixes for v2.0.0 Collections

- **Decision**: Applied 9 bug fixes identified in a pre-merge audit before merging `feat/v2-collections` to `main`. Critical: (1) `cashOnHand` decrement now always uses `claimedAmount` on discrepancy handovers — `actualAmount` was used before, leaving phantom cash permanently in the collector's ledger. (2) `handoverId` is now stamped on payments at handover creation (`onHandoverCreated`), not only at verification; combined with a stricter block in `onPaymentCancelled`, this prevents cancelling a payment that is already locked in a pending-verification handover. (3) Redundant `agingBucket != 'current'` filter removed from `sendOverdueDebtEscalations` — it required a composite index that didn't exist, causing the cron to fail silently every day. High: (4) `amountFormatted` added to `payment_recorded` and `handover_pending` notification payloads; `collectorName` added to `handover_verified`/`handover_discrepancy` payloads — notification bodies were always showing empty/partial strings. Medium: (5) `installment_due` notification now routed, titled, and described in `NotificationsScreen`. (6) Admin Firestore query hoisted outside per-installment loop in `sendInstallmentDueReminders`. (7) Handover submit SnackBar now says "Handover submitted successfully" instead of the action label. (8) Batch chunked to 200 in `checkBrokenPtps` to stay under Firestore's 500-operation limit. Low: (9) Dead ternary in `receipt_pdf_service.dart` simplified; dead `setMaxCashOnHand`/`getCashOnHand` methods removed from `EmployeesRepository`; `PaymentModel.isCancelable` now also checks `handoverId == null`; `stale_cash_admin` notification body fixed to use available CF data fields. Test callables added for `checkBrokenPtps` and `sendOverdueDebtEscalations`.
- **Reason**: Feature is financially sensitive; a full pre-merge audit was performed at owner request before the feature branch merges to main. The discrepancy cashOnHand issue (previously deferred in this log on 2026-06-28) was confirmed by owner smoke test and resolved by choosing option A: decrement by `claimedAmount` always. The discrepancy amount is preserved in the handover doc and `collection_logs` for reconciliation.
- **Impact**: `onHandoverCreated` now writes one update per payment in `paymentIds` (stamps `handoverId`). `onPaymentCancelled` block logic is now based on `handoverId` field presence rather than fetching the handover to check its status. `sendOverdueDebtEscalations` no longer needs the `[status, agingBucket]` composite index. All 7 PRs on `feat/v2-collections` are now ready for owner smoke test and merge.
- **Owner**: Mohamed Odeh.
- **Related**: `functions/lib/collections/handover-triggers.js`, `payment-triggers.js`, `schedulers.js`, `installment-triggers.js`, `functions/index.js`, `lib/.../payment_model.dart`, `notifications_screen.dart`, `handover_init_screen.dart`, `receipt_pdf_service.dart`, `employees_repository.dart`.

## 2026-06-23 — Sign Out of Other Devices: createCustomToken + IAM Service Account Token Creator role

- **Decision**: `revokeUserSessions` CF now calls `admin.auth().createCustomToken(uid)` after `revokeRefreshTokens` and returns the custom token to the client. The Flutter client calls `FirebaseAuth.instance.signInWithCustomToken(customToken)` to give the current device a new refresh token that post-dates the revocation. The Compute Engine default service account (`112459870697-compute@developer.gserviceaccount.com`) was granted the `Service Account Token Creator` role (`roles/iam.serviceAccountTokenCreator`) in Google Cloud Console to enable the signing operation.
- **Reason**: `revokeRefreshTokens` invalidates ALL refresh tokens, including the calling device's. The previously attempted `getIdToken(true)` approach failed because it tried to exchange the now-revoked refresh token for a new ID token — the server rejects this, causing a false "failed" error while the revocation had already succeeded, leaving the current device also signed out within ~1h. `signInWithCustomToken` is the canonical Firebase solution: the custom token is created server-side after revocation, so it is not covered by the revocation; calling it on the client gives the current device a brand-new refresh token. The IAM role is required because Cloud Functions gen2 uses the Compute Engine default service account, which does not have `iam.serviceAccounts.signBlob` by default (unlike gen1's App Engine service account which had `editor` role).
- **Impact**: `revokeUserSessions` response shape changed from `{success: true}` to `{success: true, customToken: string}`. The IAM role is a one-time Google Cloud Console change; no code deploy re-run is needed for the IAM change. If the custom token creation fails (IAM not granted), the CF throws `INTERNAL` and the client shows "failed" — the revocation may or may not have completed depending on which line threw. Diagnosis: `revokeUserSessions` CF logs distinguish `revokeRefreshTokens failed` vs `createCustomToken failed` via separate `console.error` calls.
- **Owner**: Mohamed Odeh.
- **Related**: `functions/lib/users.js`, `lib/features/auth/presentation/cubit/auth_cubit.dart`, `feat/v1.9.0` branch, SESSION_LOG 2026-06-23.
