...
## 2026-05-16 — Attendance architecture stabilization: sessions model, correction semantics, reports

- **Decision**: (1) **Sessions model**: replace the single `checkInAt`/`checkOutAt` day-record with a `sessions` array on the same flat `attendance/{userId}_{YYYY-MM-DD}` document. Each element is `{checkInAt, checkOutAt?, durationMinutes?}`. Unlimited sessions per day; only guard is "cannot check-in while a session is open." `totalDurationMinutes` precomputed on root. `checkInAt`/`checkOutAt` are now computed getters in Flutter (`sessions.first.checkInAt`, `sessions.last.checkOutAt`). (2) **Correction semantics**: remove `status: 'manual'` as a value. Status is now `present | absent` only (operationally, was this person here?). Admin corrections set `isCorrected: true`, `correctedBy`, `correctedAt`, `notes` — separate from status. `adminCorrectAttendance` callable no longer accepts `fields.status`. Old documents with `status: 'manual'` render gracefully via backward-compat mapping in `AttendanceModel.fromMap` (treated as `present` + `isCorrected: true`). (3) **Reports semantics**: remove daily roster from Reports screen. Reports attendance section shows monthly attendance for the selected employee + month (same pickers). Employee monthly attendance also accessible via a new drawer entry (`EmployeeMonthlyAttendanceScreen`). (4) **Index deployment fix**: root cause of "employee history not visible" was `firebase.json` missing `"indexes": "firestore.indexes.json"` — the composite index was never deployed. Fixed by wiring `firebase.json`. Secondary fix: `AttendanceScreen` history section now shows error state when `historyStatus == error` (was silently showing "no records").
- **Reason**: Real-device testing after P1–P3 revealed: (a) the single-session model blocks split shifts, leave-and-return, and evening work — realistic for the team's operational patterns; (b) `status: 'manual'` conflates operational status with correction provenance — this creates ambiguity for future lateness logic (`late` would collide with `manual` semantically); (c) Reports daily roster is the wrong grain for a reporting screen — month-level per-employee view is the operationally useful one; (d) the missing index caused a silent query failure masked by `catch (_)`. All five findings were discovered pre-production, making now the right time to correct rather than accumulate rework.
- **Impact**: Cloud Functions `recordAttendance` and `adminCorrectAttendance` rewritten for sessions model. `sendDailyAbsenceMarker` updated to write new document structure. `AttendanceModel` field changes: `sessions`, `totalDurationMinutes`, `isCorrected`, `correctedAt` added; `checkInAt`/`checkOutAt` become computed getters; `biometricVerified` removed (never surfaced). New `AttendanceSession` value class. `AttendanceRepository.fetchMonthlyAttendance` added. `AttendanceCubit.loadMonthlyAttendance` added. New `EmployeeMonthlyAttendanceScreen`. 5 new translation keys (parity 281 → 286). Firestore rules unchanged. Post-merge user action: `firebase deploy --only firestore:indexes,functions,firestore:rules`.
- **Owner**: Mohamed Odeh.
- **Related**: `CURRENT_TASK.md` (stabilization spec), PRs #35–#37 (P1–P3 + supplement), `BACKLOG.md` item #14.

## 2026-05-16 — local_auth authenticates against OS device credential, not app password

- **Decision**: Document explicitly that `local_auth` on all platforms authenticates the user against their **OS/device credential** (biometric fingerprint, Face ID, device PIN, pattern, or passcode) — not the app account password (Firebase Auth). The attendance callable is only invoked after this OS-level challenge succeeds in the Flutter client.
- **Reason**: Real-device testing showed the check-in/out succeeded regardless of the app password being "wrong," which caused confusion. This is correct behavior: Firebase Auth handles identity at login; `local_auth` handles **physical presence** at the moment of an attendance event. An employee cannot check in remotely or hand their device to someone else, because the check-in requires their device's biometric/PIN at that moment. Emulator behavior: some emulators auto-approve biometric challenges; some require the Extended Controls panel to enroll a fingerprint first — this is a platform simulator artifact, not a code bug.
- **Impact**: Documentation only — no code changes. Testers should not interpret emulator auto-approval as a security gap; on real devices the OS challenge is mandatory. Future reviewers should not confuse `biometricOnly: false` (allows PIN/pattern fallback) with "app password is accepted" — it is device credential, always.
- **Owner**: Mohamed Odeh.
- **Related**: `lib/features/attendance/presentation/screens/attendance_screen.dart` (biometric logic), `docs/ai-workflow/DECISIONS_LOG.md` 2026-05-15 (original architecture decision).
